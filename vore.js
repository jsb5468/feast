...
function Creature(name = "Creature", str = 10, dex = 10, con = 10) {
  ...
  this.finishCombat = function() {
    return [this.description("The") + " dabs so hard your brain explodes."];
  };
...
function Fam() {
  ...
  this.attacks.push(new bees(this));

  this.prefs.prey = false;
  this.prefs.grapple = false;
  this.flags.beeline = 0;
}
...
function bees(attacker) {
  return {
    attackPlayer: function(defender) {
      if (attacker.flags.beeline >= 1363) {
        defender.health = -100;
        return ["BUT IT GETS FASTER EVERY TIME THEY SAY BEE"];
      }
      return [beeMovie[attacker.flags.beeline++]];
    },
    priority: 1,
